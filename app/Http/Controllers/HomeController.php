...
class HomeController extends Controller
{
    public function index()
    {
        ...
        // why us
        if (Navigation::query()->where('nav_category', 'Home')->where('nav_name', 'LIKE', "%why-choose-us%")->where('page_type', 'Group')->latest()->first() != null) {
            $why_id = Navigation::query()->where('nav_category', 'Home')->where('nav_name', 'LIKE', "%why-choose-us%")->where('page_type', 'Group')->latest()->first()->id;
            $why_us_parent = Navigation::find($why_id);
            //   return $why_us_parent; 
            $why_us_id = Navigation::query()->where('parent_page_id', $why_id)->get()->take(6);
            //   return $why_us_id; 
        ...
        // home our services
        if (Navigation::query()->where('nav_category', 'Home')->where('nav_name', 'LIKE', "%our-services%")->where('page_type', 'Group')->latest()->first() != null) {
